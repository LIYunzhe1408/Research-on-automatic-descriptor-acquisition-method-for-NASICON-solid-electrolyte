...
public class SessionInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        //session过期
        if (request.getSession(false)==null||request.getSession(false).getAttribute("userLoginNo")==null) {
        ...
        //用户被管理员禁用校验
        String loginNo= String.valueOf(request.getSession(false).getAttribute("userLoginNo"));
        if(!userService.getUserStatus(loginNo).equals(StatusCodeEnums.ACTIVE.getCode())){
        ...
        //todo 用户页面功能权限校验：需要新建用户-页面-操作的对应表解决或者直接前端解决，可以在想想（与同一浏览器session覆盖有关），不解决也问题不大


//        System.out.println("1"+request.getSession(false).getAttribute("userLoginNo"));
        return true;
    }
}
