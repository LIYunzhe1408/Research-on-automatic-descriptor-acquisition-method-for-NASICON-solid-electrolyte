...
def clean_text(r):
    ...
    r = r.replace('’l', '-1' )
    r = r.replace('Ric', 'R3c' )
    r = r.replace('sp. gr.', 'sp gr')
    r = r.replace( 'E"' , 'Eu' )
    r = r.replace('Km', 'K alpha')
    r = r.replace('Jr', '+')
    r = r.replace('\n\n', '||')
    r = r.replace('\n', ' ')             # 合并按照行识别的内容
    r = r.replace('||', '\n\n')      
    r = re.sub('(\.)([A-Z])',r'\1 \2',r)   #. 首字母中间 加上空格
    r = r.replace('. ', '. \n\n')        # 按照. 分句 #修改1 句号后不分句
    #或者 r = re.sub('(\.)([A-Z])',r'\1 \n\n \2',r)
    # r = re.sub('(\. )([A-Z])',r'\1 \n\n \2',r)
    r = r.replace('—', '-')
    ...
    for par in r.split('\n\n'):
        ...
        par = re.sub('-\n\n', '', par)
        par = re.sub('-\n', '', par)
        par = re.sub('\[[0-9]{1,3}\]', '', par)      # 删除[14]和[1]
        par = re.sub('\([0-9]{1,3}\)', '', par) 
        ...
        if 'ACKNOWLEDGEMENTS' in par:
            break
        if len(par.split(' ')) > 5:
            if (par[-2] == '.' and par[-1] == ' ') or par[-1] == '.':  ##长空换行
                q = q + par + '\n\n'
            else:
                q = q + par
...
if __name__ == '__main__':
    ...
    for i in range(len(os.listdir(sys.argv[1].split('.')[0] + '/'))):
        ...
        image = Image.open(sys.argv[1].split('.')[0]+'/'+str(i+1)+'.png') #文件转换
        content = tesserocr.image_to_text(image) #打开文件
        content1 += content
    content1 = clean(content1)         # 清洗文本

    with open("E:\\File\\1_InnovateProj\\Algorithm\\NER\\result_literatures\\" + sys.argv[1].split('.')[0][40:] + '.txt', 'w', encoding='utf-8') as f: #输入 创建txt
        f.write(content1)
        f.close()
    # 删掉生成的图片
